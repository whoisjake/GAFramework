...
// modified: PJ
// date:   May 02, 2002
// notes:  Included many more board analysis functions
//***********************************************
   ...
   // checkers that i and j are between 0 and 7 inclusive
      ...
      private boolean isEmpty(int i, int j)
      {
		...
		public int redInCorner()
		{
			int count=0;
			
			for (int j=0; j<8; j++)
			{
			 	if ((position[0][j] == RED) && (position[0][j] != RKING))
					count++;
				
				if ((position[7][j]==RED) && (position[7][j]!=RKING))
					count++;
			}
			return count;
		}

		public int blackInCorner()
		{
			int count=0;
			
			for (int j=0; j<8; j++)
			{
			 	if ((position[0][j] == BLACK) && (position[0][j] != BKING))
					count++;
				
				if ((position[7][j]==BLACK) && (position[7][j]!=BKING))
					count++;
			}
			return count;
		}

		public int rKingInCenter()
		{
			int count =0;
			for (int i=0; i<8; i++)
         {
      ...
      public int blackPieces()
      {
		...
		public int potentialRKing()
		{
			int count=0;
			for (int i=0; i<8; i++)
			{
			 	if ((position[i][1]==RED) || (position[i][2]==RED))
					count++;
			}
			return count;
		}

		public int potentialBKING()
		{
			int count=0;
			for (int i=0; i<8; i++)
			{
				if ((position[5][i]==BLACK) || (position[6][i]==BLACK))
					count++;
			}
			return count;
		}
   	
		//potential pieces that can be captured by black
		public int captureByBlack()
		{	
			int count =0;
		
			/*for (int k=0; k<8; k++)
			{
				if (position[i][0]==BLACK)
				{

					if ((position[i+1][j+1]==RED || (position[i+1][j+1]==RKING))
						count++;
					if ((position[i-1][j+1]==RED) || (position[i-1][j+1]==RKING))
						count++;
				}
				else if (position[i][0]==BKING)
				{
					if ((position[i+1][j+1]==RED || (position[i+1][j+1]==RKING))
						count++;
					if ((position[i-1][j+1]==RED) || (position[i-1][j+1]==RKING))
		...
		public int captureByRed()
		{
			...
			for (int i=1; i<7; i++)
			{
				for (int j=1; j<7; j++)
				{
					if (position[i][j]==RED)
					{
						if ((position[i+1][j+1]==BLACK) || (position[i+1][j+1]==BKING))
							count++;
						if ((position[i-1][j+1]==BLACK) || (position[i-1][j+1]==BKING))
							count++;
					}
