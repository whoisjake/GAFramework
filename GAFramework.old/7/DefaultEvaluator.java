...
public class DefaultEvaluator implements Evaluator
{
	...
	private int RANDOM_WEIGHT=100; // weight factor
	private int color;
	...
	public int evaluate(Board position)
	{
    	...
    	for (int i=0; i<8; i++)
		{
      	for (int j=0; j<8; j++)
			{
				if (position.getPiece(i,j) == Board.RED)
	      	{
					score-=CHECKER;
					score-=POS*j*j;
	      	}
	    		else if (position.getPiece(i,j) == Board.RKING)
				{
	      		score-=KING;
		  			if (i==0 || i==7)
			  			score += EDGE;
				...
				else if (position.getPiece(i,j) == Board.BKING)
				{
	      		score+=KING;
		  			if (i==0 || i==7)
			  			score -= EDGE;
	    		...
	    		else if (position.getPiece(i,j) == Board.BLACK)
	    		{
					score+=CHECKER;
					score+=POS*(7-j)*(7-j); 
				}
